...
Three shapes. **Loose files** beside the executable — easy to inspect, easy for a player to break. A **single `.ori`** next to the exe, which is memory-mapped so mounting costs essentially nothing. Or **embedded in the executable**, and the game is literally one file.
...
**Generate a patch against a real previous build at least once before launch.** Patch generation is the kind of pipeline step that works perfectly until the first time you genuinely need it at speed.
