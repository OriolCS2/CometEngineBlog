...
**The server crashed under request bursts.** A tool that pumps frames — a scene load, an import — re-entered the request loop and reallocated the client list underneath the outer call.

All three are the same category: paths that only get exercised when something drives them faster and more mechanically than a person would. That is a genuinely useful property to have in a codebase.
...
**Determinism.** A model driving an editor makes mistakes. Everything it does goes through the same undo system a human's actions do, and nothing bypasses validation — the tools are the same commands the UI issues.
