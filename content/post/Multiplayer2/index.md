...
That is a client sending its input to the host: an unreliable RPC for the stick, a reliable one for the fire button. The `IsMultiplayerAuthority()` guard at the top of each is not optional, because an RPC is a method anyone can call.
...
**Authority**, either `any_peer` or `authority`. Who is allowed to invoke this? A client saying "I picked up the item" is `any_peer`. A server saying "the round ended" is `authority`. Getting this wrong is how a cheat client rewrites your game state.

**Call target**, either `call_local` or `call_remote`. Does the caller also run it? Fire a weapon and you want local (instant feedback) plus remote. Apply damage and you may want remote only.
...
Both behaviours sit on the ship: the synchronizer above, the spawner below. Neither has much of a surface, which is the point, because the configuration lives on the script's fields.
...
Two modes. **Always** sends every tick. It is the simplest and the most bandwidth, and it is right for things that really do change constantly. **On Change** delta-compresses and sends only what moved, which is right for almost everything else.
...
The part people underestimate, every single time, is **late joiners**.
...
**Interest management** is `SetVisibilityFor` on a synchronizer, per peer and per entity. A player on the far side of the map does not need updates about things they cannot see.
