...
The concept is simple enough to explain in a sentence: an InstanciableEntity is an entity you saved as an asset, so you can place it as many times as you like and edit all of the copies from one place.

The consequences of that sentence took me about three rewrites to get right.

## Making one

Build an entity in a scene — a coin, say. Transform, Sprite Renderer, a collider set to trigger, a little script. Then drag it from the Hierarchy into the Project panel.

That is it. You now have a `.cometInstanciableEntity` asset, and the entity still in your scene is no longer just an entity — it is an **instance** of that asset. It remembers where it came from.

![Instances in the scene](instances.png)

Drop that asset into the scene eight more times and you have eight instances. Open the asset, change the sprite, save, and all eight change. This is the entire point and it is worth being unromantic about it: the value here is not clever architecture, it is that you do not have to select ninety-three coins to make them all slightly bigger.
...
This is the part that matters, and it is not obvious from the outside.

![What it stores](what-it-is.png)
...
- Changing the asset genuinely changes every instance everywhere, including in scenes that are not currently open, because they never had their own copy of the data to go stale.
...
Every turret has to update. Every room containing those turrets has to update. Every scene containing those rooms has to update. And crucially — none of them may lose the changes they made themselves. If one particular turret in one particular room was rotated 40 degrees and had its fire rate doubled, that has to survive an edit three levels below it.

Comet's answer is that every entity inside an instance carries a small set of identifiers: which asset it belongs to, which asset it is *inside*, which asset is outermost, and what its parent asset is. Those four values are what lets the engine walk a three-level-deep structure and work out, for any given entity, which asset it should be inheriting from and which changes belong to it.

The original version of this system, back in 1.x, only supported one level. Nesting was added in 2.0 and it was the single largest change in that release that nobody outside could see. It also produced the largest crop of bugs — the 2.4.1 changelog contains the wonderfully understated line "Multiples errors on InstanciableEntities", which is doing a lot of work to describe a fortnight.
...
**You cannot delete a child of an instance.** Not really. If a turret asset has a barrel, and you delete the barrel on one instance, the engine cannot simply remove it — the asset still says there is a barrel, and re-stamping would bring it back. What actually happens is a *tombstone*: the instance records that this particular child is no longer valid. It works, and it is the correct answer, but it means the deletion is a stored opinion rather than an absence.
